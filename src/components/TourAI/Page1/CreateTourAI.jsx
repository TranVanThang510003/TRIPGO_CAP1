...
const CreateTourAI = () => {
  ...
  const [selectedCity, setSelectedCity] = useState({ name: "", imageUrl: "" });
  ...
  const handleImageClick = (name, imageUrl) => {
    setSelectedCity({ name, imageUrl });
    setSearchValue(name);
    setIsModalOpen(false);
  };
    ...
    <div className="container">
          ...
          <div className="product_item" key={item.id}>
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                borderRadius: "20px",
                display: "flex", // Apply flex to the button to align content
                alignItems: "center", // Align items vertically
                justifyContent: "center", // Center the text and icon horizontally
                position: "relative",
                transition: "background-color 0.3s ease", // Only background color change
              }}
            ...
            >
              ...
              {activeButton === item.id && (
                <FiCheck
                  style={{
                    marginLeft: "10px", // Space between the text and icon
                    fontSize: "1.5rem", // Adjust icon size
                    display: "inline-flex", // Ensure icon stays inline with text
      ...
      <div className="suggestions">
        {filteredCities.map((item) => (
          <div
            ...
            key={item.id}
            onClick={() => handleImageClick(item.name, item.imageUrl)}
      ...
      {isModalOpen && (
        <SearchModal
          ...
          onChange={handleInputChange}
          onSelect={(result) => {
            setSelectedCity({ name: result.name, imageUrl: result.imageUrl });
