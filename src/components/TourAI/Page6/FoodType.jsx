...
const FoodType = () => {
  ...
  const { city, dates, dayDifference } = location.state || {}; // Ensure dates and city are destructured properly

  // Debugging to check incoming state
  console.log("Location State:", location.state);
  console.log("Selected Dates in FoodType:", dayDifference);
  ...
  const handleBack = () => {
    navigate("/hobbyai", { state: { city, dates } }); // Ensure dates are passed back
  };

  const handleContinue = () => {
    if (city) {
      navigate("/recommendations", {
        state: { city, dates, dayDifference: dayDifference },
      });
    } else {
