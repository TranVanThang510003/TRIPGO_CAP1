...
const useCreateHotelForm = (initialData = {}) => {
    // Dữ liệu tỉnh/quận/xã
    const [provinces, setProvinces] = useState([]);
    const [districts, setDistricts] = useState([]);
    const [wards, setWards] = useState([]);

    // Các state với giá trị mặc định từ `initialData`
    ...
    const [selectedWard, setSelectedWard] = useState(initialData.ward || '');
    ...
    const fetchWards = async (districtId) => {
        try {
            ...
            setWards(wardsData);
            return wardsData; // Trả về danh sách xã/phường
        } catch (error) {
    ...
    const handleProvinceChange = (e) => {
        const provinceId = e.target.value;
        setSelectedProvince(provinceId);
        setSelectedDistrict(''); // Reset district và ward
        setSelectedWard('');
        setDistricts([]); // Xóa danh sách quận
