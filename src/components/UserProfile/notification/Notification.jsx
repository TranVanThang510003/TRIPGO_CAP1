                            ...
                            <div key={tour.id} className="bg-blue-100 border-l-4 border-blue-500 p-4">
                                <div className="flex items-start">
                                    <div className="flex-shrink-0">
                                        <div className=" text-3xl  font-bold bg-blue-500 text-white w-12 h-12 rounded-full flex justify-center items-center">
                                            T
                                        </div>
                                    </div>
                                    <div className="ml-4 flex justify-between w-full">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">Ngày hết hạn: {new Date(tour.scheduleData.slice(-1)[0].endDate).toLocaleDateString()}</p>
                                        </div>
                        ...
                        {upcomingTours.length > 0 ? (
                            upcomingTours.map(order => (
                                <div key={order.bookingId} className="bg-yellow-100 border-l-4 border-yellow-500 p-4">
                                    <div className="flex items-start">
                                        <div className="flex-shrink-0">
                                            <div className=" text-3xl  font-bold bg-yellow-500 text-white w-12 h-12 rounded-full flex justify-center items-center">
                                                T
                                            </div>
                                        </div>
                                        <div className="ml-4 flex justify-between w-full">
                                            <div>
                                                <p className="text-sm font-medium text-yellow-900">{order.tourName} sắp đến ngày khởi hành</p>
                                                <p className="text-sm text-gray-500">Số lượng người: {order.adultCount} người lớn, {order.childCount} trẻ em</p>
                                                <p className="text-xs text-gray-500">Ngày khởi hành: {new Date(order.departureDate).toLocaleDateString()}</p>
                                            </div>
                                            <div className="flex items-center justify-start mt-2">
                                                <button
                        ...
                        {finishedTours.length > 0 ? (
                            finishedTours.map(order => (
                                <div key={order.bookingId} className="bg-green-100 border-l-4 border-green-500 p-4">
                                    <div className="flex items-start">
                                        <div className="flex-shrink-0">
                                            <div className=" text-3xl font-bold bg-green-500 text-white w-12 h-12 rounded-full flex justify-center items-center">
                                                T
                                            </div>
                                        </div>
                                        <div className="ml-4 flex justify-between w-full">
