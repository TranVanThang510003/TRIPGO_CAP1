...
const CampingList = () => {
  ...
  // Goi API
  const [dataCamping, setDataCamping] = useState();
  useEffect(() => {
    try {
      const fetchTour = async () => {
        const response = await fetch("http://localhost:8000/campings");
        const result = await response.json();
        console.log({ result });
        setDataCamping(result);
      };
      fetchTour();
    } catch (error) {
  ...
  // Tính toán số trang
  const totalPages = Math.ceil(dataCamping?.length / CAMPING_PER_PAGE);

  // Lấy danh sách khách sạn cho trang hiện tại
  const currentCamping = (dataCamping || []).slice(
